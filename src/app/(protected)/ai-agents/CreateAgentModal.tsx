      ...
      <div className="bg-white rounded-xl shadow-xl p-8 w-full max-w-md relative">
        ...
        {currentStep === 2 && (
          <form className="space-y-5" onSubmit={handleSave}>
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700">Cost to Run (Credits)</label>
              <div className="text-xs text-gray-500 mb-2">
                Set how many credits users will pay to execute this workflow. Leave as 0 for free workflows.
              </div>
              <input
                className="w-full border border-gray-300 px-4 py-3 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-colors"
                value={credits}
                onChange={e => setCredits(e.target.value)}
                placeholder="e.g., 5 (for 5 credits per execution)"
                type="number"
                min="0"
                step="1"
              />
            ...
            <div className="flex gap-3 mt-4">
              ...
              <button
                type="submit"
                className="bg-purple-600 text-white px-6 py-2 rounded font-bold hover:bg-purple-700 flex-1"
                disabled={credits === "" || credits === undefined}
              >
                Save
              </button>
            </div>
